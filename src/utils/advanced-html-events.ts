...
export const waitForSelectorInFrame = (frame: HTMLIFrameElement, selectors: string[]): Promise<void> => {
...
export const waitForSelectorsInDoc = (doc: Document, selectors: string[]): Promise<void> => {

    return new Promise(async (resolve, reject) => {

        try {
            await pollUntil(() => {
                return doAllSelectorsExistOnDocument(doc, selectors)
            })
            resolve()
        } catch (e: TimeoutError | any) {
            reject(new Error("Could not resolve element with selectors!"))
